...
$$
l(f|y) = \text{const.} - \frac{1}{2} \sum_{i=1}^n\sum_{j=1}^n \psi_{ij}
...
\cI_f = -\E\nabla^2 L(f|y),
$$
so the Fisher information between two point evaluation functionals of is 
$$
\cI\big(f(x), f(x') \big) = \cI_f\big(h(\cdot,x), h(\cdot,x') \big) = \sum_{i=1}^n\sum_{j=1}^n \psi_{ij}h(x,x_i) h(x',x_j).
$$
