...
In either estimation method, $V_y^{-1}$ is computed and takes $O(n^3)$ time; storage requirements is $O(n^2)$. 

\vspace{1em}

Trick: low-rank matrix approximations.
Suppose $H\approx QQ^\top$, where $Q \in \bbR^{n\times m}$ where $m \ll n$.
Then, using the Woodbury matrix identity,
$$
V_y^{-1} = (H\Psi H + \Psi^{-1})^{-1} \approx \Psi - \Psi Q\big((Q^\top \Psi Q)^{-1} + Q^\top\Psi Q\big)^{-1} Q^\top \Psi
$$
is a much cheaper $O(nm^2)$ operation.
